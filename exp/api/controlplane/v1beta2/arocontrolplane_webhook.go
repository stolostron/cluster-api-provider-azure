...
func (m *AROControlPlane) validateEncryptionKeyVersion(spec map[string]interface{}, basePath *field.Path) field.ErrorList {
	...
	// KMS encryption is configured, validate that keyVersion is specified
	keyVersion, hasKeyVersion := kms["keyVersion"].(string)
	if !hasKeyVersion || keyVersion == "" {
		allErrs = append(allErrs, field.Required(
			basePath.Child("spec", "properties", "etcd", "dataEncryption", "customerManaged", "kms", "keyVersion"),
			"keyVersion is required when identityRef is not set - CAPZ cannot auto-create or propagate the encryption key without Azure credentials",
		))
	}
