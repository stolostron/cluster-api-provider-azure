...
func (s *aroControlPlaneService) reconcileResources(ctx context.Context) error {
	...
	if err == nil {
		...
		if err == nil {
			// Found v20260901preview version
			...
			return errors.Wrap(err, "failed to get HcpOpenShiftCluster (v20260901preview)")
		}
	} else {
	...
	if !hasReadyNodePool {
		...
		if err := s.kubeclient.List(ctx, nodePoolListV2, client.InNamespace(s.scope.Namespace())); err != nil {
			...
			if !apierrors.IsNotFound(err) && !meta.IsNoMatchError(err) && !isSchemeError(err) {
				return nil, false, fmt.Errorf("failed to list HcpOpenShiftClustersNodePool resources (v20260901preview): %w", err)
			}
		} else {
			log.V(4).Info("Checking node pool readiness (v20260901preview)", "nodePoolCount", len(nodePoolListV2.Items))
			for _, nodePool := range nodePoolListV2.Items {
				for _, condition := range nodePool.Status.Conditions {
					if condition.Type == asoconditions.ConditionTypeReady && condition.Status == metav1.ConditionTrue {
						hasReadyNodePool = true
						log.V(4).Info("Found ready node pool (v20260901preview)", "name", nodePool.Name)
						break
					}
				}
				if hasReadyNodePool {
